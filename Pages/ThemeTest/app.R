...
addUser <- function(email, firstName, lastName, organisation){
  ...
  if(nrow(user) > 0 ){
    msg <- paste0("The email address already has an associated API Key.<br> Please contact <a href='mailto:ross.searle@csiro.au?subject=Trouble%20Registering%20for%20the%20SoilDataFederator'> ross.searle@csiro.au</a> to sort this out.")
  }else{
...
ui <- fluidPage(
  ...
  tags$head(
    ...
    includeHTML( paste0(rootDir,'/Docs/TERNLandscapesHeadNoTags.html'))
    ),



  includeHTML( paste0(rootDir,'/Docs/TERNLandscapesTopBodyNoTags.html')),

  tags$div(class="container", HTML("<br>"),
           ...
           tags$div(id="content",
                    ...
                    uiOutput("P1"),
                    br(),
                    tags$table(align="left", width="300px",
                               tags$tr(width = "60%",
                               ...
                               tags$tr(width = "60%",
                                       #tags$td(width = "30%", ""),
                                       tags$td(width = "30%", colspan="2", HTML(""), div(style = "font-size:14px; font-weight:bold; color:#ebb72f; text-align: center;", uiOutput("SubmitMessage"),  HTML(""))))
                    ),
                    ...
                    includeHTML(paste0(rootDir, '/Docs/TERNLandscapesBottomBodyNoTags.html'))
           )
  )
