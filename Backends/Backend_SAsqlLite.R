  ...
  for (i in 1:nrow(propRecs)) {
    ...
    if(propertyType==PropertyTypes$LaboratoryMeasurement){
      ...
      if(nrow(fdf) > 0){
        ...
        date <- as.Date(rd, format = "%d/%b/%Y")
        odate <- format(date, format="%d-%m-%Y")
        oOutDF <- generateResponseDF(DataSet, paste0(fdf$id, '_', fdf$site_id), fdf$horizon_id, odate, fdf$longitude, fdf$longitude, fdf$`upper_depth:1` , fdf$`lower_depth:1`, propertyType, sProp,fdf$lab_result_value, units)
        lodfs[[i]] <- oOutDF
      }else{
      ...
      if(tabLev == 4){
        ...
        date <- as.Date(rd, format = "%d/%b/%Y")
        odate <- format(date, format="%d-%m-%Y")
        oOutDF <- generateResponseDF(DataSet, paste0(fdf$id, '_', fdf$site_id), fdf$horizon_id, odate, fdf$longitude, fdf$longitude, fdf$upper_depth , fdf$lower_depth, propertyType, sProp, fdf[, 9], units)
