  ...
  lodfs <- list(length(nativeProps)) 
    ...
    for (i in 1:length(nativeProps)) {
      ...
      if(length(sd) > 0){
          ...
          oOutDF <- generateResponseDF(OrgName, paste0(fdf$projectCode , '_', fdf$siteId, '_', fdf$observationNumber ), fdf$sampleNumber, fdf$analysisDate, fdf$longitude , fdf$latitude, 
